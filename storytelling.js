(() => {
  ...
  /* Retire the older decorative field without touching its feature code. */
  ...
  /* 01 — Background storytelling: chaos → structure → flow → intelligence → result. */
  const stageMap = [
  ...
  if (!reduced) {
    ...
    const resize = () => {
      ...
      for (let i = 0; i < particleCount; i += 1) {
        particles.push({ x: Math.random() * width, y: Math.random() * height, px: 0, py: 0, seed: Math.random() * 20, speed: .35 + Math.random() * .65, age: Math.random() * 220 });
      }
    };

    const reset = p => {
      const key = activeStage?.key || 'chaos';
      if (key === 'final' || key === 'growth') {
        p.x = Math.random() * width * .75;
        p.y = height + Math.random() * 80;
      } else if (key === 'structure' || key === 'flow') {
    ...
    const vector = p => {
      ...
      if (key === 'chaos') {
        angle = -1.05 + Math.sin(nx * 8 + time * .00045 + p.seed) * .52 + Math.cos(ny * 5 - time * .00025) * .22;
        boost = .75;
      } else if (key === 'signal') {
        const lane = Math.round(nx * 4) / 4;
        angle = -1.16 + (lane - nx) * .88 + Math.sin(time * .0004 + p.seed) * .12;
      } else if (key === 'structure') {
        angle = -.18 + Math.sin(ny * 6 + p.seed) * .07;
        boost = 1.08;
      } else if (key === 'measure') {
        angle = -.7 + Math.sin(nx * 5 + p.seed) * .12;
        boost = 1.16;
      } else if (key === 'flow') {
        const targetY = height * (.25 + (Math.round(ny * 3) / 3) * .62);
        angle = Math.atan2(targetY - p.y, width + 40 - p.x) + Math.sin(time * .00035 + p.seed) * .05;
        boost = 1.12;
      } else if (key === 'growth') {
        angle = Math.atan2(-height * .15 - p.y, width * .86 - p.x) + Math.sin(p.seed + time * .0003) * .08;
        boost = 1.2;
        ...
        const dx = cx - p.x;
        const dy = cy - p.y;
        angle = Math.atan2(dy, dx) + .34 * Math.sin(p.seed + time * .00035);
        boost = .9;
      } else if (key === 'final') {
        const tx = width * (mobile ? .72 : .83);
        const ty = height * .26;
    ...
    const drawArrow = (x, y, angle, alpha) => {
      ...
      ctx.beginPath();
      ctx.moveTo(x, y);
      ctx.lineTo(x + Math.cos(angle + Math.PI - .48) * size, y + Math.sin(angle + Math.PI - .48) * size);
      ctx.moveTo(x, y);
      ctx.lineTo(x + Math.cos(angle + Math.PI + .48) * size, y + Math.sin(angle + Math.PI + .48) * size);
    ...
    const render = now => {
      ...
      const key = activeStage?.key || 'chaos';
      const baseAlpha = mobile ? .095 : .15;

      particles.forEach((p, index) => {
        p.px = p.x;
        p.py = p.y;
        const v = vector(p);
        const step = p.speed * v.boost * (mobile ? 1.25 : 1.65);
        p.x += Math.cos(v.angle) * step;
        p.y += Math.sin(v.angle) * step;
        p.age += 1;

        const gradient = ctx.createLinearGradient(p.px, p.py, p.x, p.y);
        gradient.addColorStop(0, `rgba(112,244,208,${baseAlpha * .22})`);
        gradient.addColorStop(1, `rgba(183,255,55,${baseAlpha})`);
        ctx.beginPath();
        ctx.moveTo(p.px, p.py);
        ctx.lineTo(p.x, p.y);
        ctx.strokeStyle = gradient;
        ctx.lineWidth = index % 5 === 0 ? .95 : .55;
        ctx.stroke();
  ...
  /* 02 — One energy line crossing the whole experience. */
  const spine = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  spine.setAttribute('class', 'energy-spine');
  spine.setAttribute('viewBox', '0 0 100 100');
  ...
  const spinePath = $('path', spine);
  const spineLength = spinePath?.getTotalLength?.() || 160;
  if (spinePath) {
    spinePath.style.strokeDasharray = `${spineLength}`;
    spinePath.style.strokeDashoffset = `${spineLength}`;
  }

  /* 03 + 06 — Scene-like section transitions + kinetic typography. */
  const kineticLabels = new Map([
    ['problem', 'GARGALO'],['build-section', 'SISTEMA'],['business-cases', 'IMPACTO'],['method', 'FLUXO'],['proof-section', 'RESULTADO'],['fit-section', 'DECISÃO'],['scanner-section', 'DIAGNÓSTICO'],['contact-section', 'ESCALA']
  ]);
  $$('.section').forEach(section => {
    const classKey = [...kineticLabels.keys()].find(key => section.classList.contains(key));
    if (classKey) {
      const label = document.createElement('span');
      label.className = 'story-kinetic';
      label.textContent = kineticLabels.get(classKey);
  ...
  const sectionObserver = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      ...
      section.classList.add('story-section-active', 'story-section-enter');
      setTimeout(() => section.classList.remove('story-section-enter'), 950);
    });
  }, { threshold: mobile ? .08 : .16, rootMargin: '0px 0px -8% 0px' });
  $$('.section').forEach(section => sectionObserver.observe(section));

  /* 07 — Spatial hero: lightweight layered depth, no WebGL dependency. */
  const hero = $('.hero');
  if (hero) {
    const depth = document.createElement('div');
    depth.className = 'hero-depth';
    depth.setAttribute('aria-hidden', 'true');
    depth.innerHTML = '<span class="plane-a"></span><span class="plane-b"></span><span class="plane-c"></span>';
    hero.prepend(depth);
    hero.classList.add('story-depth-active');
  }

  /* 04 — Scroll builds the system instead of only revealing it. */
  ...
  if (commandCenter) {
    ...
    const systemObserver = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        if (!entry.isIntersecting) return;
        setTimeout(() => commandCenter.classList.add('system-built'), mobile ? 120 : 260);
  ...
  /* 05 — Business cases as BEFORE → SYSTEM → AFTER micro-stories. */
  $$('.business-case').forEach(card => {
    const switcher = document.createElement('div');
    switcher.className = 'case-mode-switch';
    switcher.setAttribute('role', 'group');
    switcher.setAttribute('aria-label', 'Etapa do business case');
    ['Antes', 'Sistema', 'Depois'].forEach((label, index) => {
      const button = document.createElement('button');
      button.type = 'button';
      button.textContent = label;
      button.dataset.mode = ['before', 'system', 'after'][index];
      if (index === 2) button.classList.add('is-active');
      switcher.appendChild(button);
    });
    const lead = $('.business-case-lead', card);
    (lead || $('h3', card))?.insertAdjacentElement('afterend', switcher);
    card.classList.add('case-after');
    $$('button', switcher).forEach(button => button.addEventListener('click', () => {
      $$('button', switcher).forEach(btn => btn.classList.toggle('is-active', btn === button));
      card.classList.remove('case-before', 'case-system', 'case-after');
      card.classList.add(`case-${button.dataset.mode}`);
    }));
  });

  /* 09 — Metrics become visual objects, not only counters. */
  const addMetricViz = (container, value, kind = 'ring') => {
    ...
    viz.dataset.kind = kind;
    viz.style.setProperty('--p', '0');
    container.appendChild(viz);
    const p = clamp(Math.abs(value), 8, 100);
    const io = new IntersectionObserver(entries => {
      if (entries.some(entry => entry.isIntersecting)) {
        requestAnimationFrame(() => viz.style.setProperty('--p', String(p)));
        io.disconnect();
      }
    }, { threshold: .4 });
  ...
  /* 08 — Opportunity Scanner: visitor becomes part of the narrative. */
  const scannerData = {
  ...
  if (scanner) {
    const options = $$('.scanner-option', scanner);
    const title = $('#scanner-result-title');
    const copy = $('#scanner-result-copy');
    const kpis = $('#scanner-result-kpis');
    let selected = 'tempo';
    const renderScanner = key => {
      selected = scannerData[key] ? key : 'tempo';
      const data = scannerData[selected];
      options.forEach(option => option.classList.toggle('is-selected', option.dataset.signal === selected));
      if (title) title.textContent = data.title;
      if (copy) copy.textContent = data.copy;
      if (kpis) kpis.innerHTML = data.kpis.map(kpi => `<span>${kpi}</span>`).join('');
    };
    options.forEach(option => option.addEventListener('click', () => renderScanner(option.dataset.signal)));
    $('#scanner-to-contact')?.addEventListener('click', () => {
      const data = scannerData[selected];
      const objective = $('#lead-form select[name="objective"]');
      if (objective) {
        const match = [...objective.options].find(option => option.textContent.trim() === data.objective);
        if (match) objective.value = match.value || match.textContent;
      }
      const challenge = $('#lead-form textarea[name="challenge"]');
      if (challenge && !challenge.value.trim()) challenge.value = `Quero avaliar um gargalo principalmente relacionado a ${selected}. Precisamos medir ${data.kpis.join(', ')} e entender o potencial de ${data.title.toLowerCase()}.`;
      $('#contato')?.scrollIntoView({ behavior: reduced ? 'auto' : 'smooth', block: 'start' });
    });
    renderScanner('tempo');
  }

  /* 10 — The whole visual system converges into the LightPath mark at the CTA. */
  const contact = $('.contact-section');
  if (contact) {
    const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    svg.setAttribute('class', 'convergence-mark');
    svg.setAttribute('viewBox', '0 0 700 700');
    svg.setAttribute('aria-hidden', 'true');
    svg.innerHTML = '<path d="M20 650 C160 560 180 470 310 390"/><path d="M95 670 C230 570 265 440 380 325"/><path d="M185 690 C320 570 390 400 470 245"/><path class="mountain-final" d="M165 560 284 398l72 84 103-174 128 252"/><path class="mountain-final" d="M185 560h386"/>';
    contact.prepend(svg);
  }

  /* Kinetic bridge reacts to scroll instead of looping independently. */
  ...
  const updateScrollStory = () => {
    ...
    const progress = clamp(scrollY / total, 0, 1);
    if (spinePath) spinePath.style.strokeDashoffset = `${spineLength * (1 - progress)}`;
    if (kineticBridge) {
      const rect = kineticBridge.getBoundingClientRect();
      const local = clamp((innerHeight - rect.top) / (innerHeight + rect.height), 0, 1);
      kineticBridge.style.setProperty('--kinetic-shift', `${(local - .5) * (mobile ? 34 : 90)}px`);
    }
    if (hero && !reduced) {
      const heroRect = hero.getBoundingClientRect();
      const p = clamp(-heroRect.top / Math.max(heroRect.height, 1), 0, 1);
      $$('.hero-depth span').forEach((plane, index) => { plane.style.translate = `${(index - 1) * p * 10}px ${p * (14 + index * 8)}px`; });
    }
    pickActiveStage();
    const stageIndex = Math.max(0, stageMap.indexOf(activeStage));
    const x = 18 + (stageIndex / Math.max(1, stageMap.length - 1)) * 66;
    const y = 68 - (stageIndex / Math.max(1, stageMap.length - 1)) * 45;
